...
extension DefaultMoviesRepository: MoviesRepository {
    func fetchCustomMoviesList(page: Int,
                               type: MoviesActionType,
                                 cached: @escaping (MoviesPage) -> Void,
                                 completion: @escaping (Result<MoviesPage, Error>) -> Void
    ) -> Cancellable? {
        
        let requestDTO = MoviesRequestDTO(query: "", page: page)
        ...
        cache.getResponse(for: requestDTO) { [weak self, backgroundQueue] result in
            ...
            switch type {
            case .upcoming:
                endpoint = APIEndpoints.getUpcomingMovies(with: requestDTO)
            case .topRated:
                endpoint = APIEndpoints.getTopRatedMovies(with: requestDTO)
            case .popular:
                endpoint = APIEndpoints.getPopularMovies(with: requestDTO)
            case .search:
                break
            }
