...
class SearchMoviesUseCaseTests: XCTestCase {
    ...
    class MoviesRepositoryMock: MoviesRepository {
        ...
        func fetchCustomMoviesList(page: Int, type: MoviesActionType, cached: @escaping (MoviesPage) -> Void, completion: @escaping (Result<MoviesPage, Error>) -> Void) -> Cancellable? {
            completion(result)
            fetchCompletionCallsCount += 1
            return nil
        }
